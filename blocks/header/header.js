...
export default async function decorate(block) {
  ...
  const navElements = [...fragment.querySelectorAll('div>ul')];
  if (navElements.length > 0) {
    const mainUl = navElements[0];
    mainUl.className = 'blcok lg:flex 2xl:ml-20';
    let i = 0;
    // eslint-disable-next-line no-restricted-syntax
    for (const child of mainUl.children) {
      child.className = 'relative group';
      if (i === 0) {
        child.children[0].className = 'ud-menu-scroll mx-8 flex py-2 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 lg:text-white lg:group-hover:text-white lg:group-hover:opacity-70 active';
      } else {
        // if nav has a submenu process it differently
        if (child.children && child.children[0].tagName === 'UL') {
          child.className = 'relative submenu-item group';
          const subMenuText = child.childNodes[0].textContent;
          const anchorElements = [...child.querySelectorAll('a')];
          let anchors = '';
          anchorElements.forEach((anchor) => {
            anchors += `<a href="${anchor.href}" class="block rounded px-4 py-[10px] text-sm text-body-color hover:text-primary dark:text-dark-6 dark:hover:text-primary">${anchor.textContent}</a>`;
          });
          child.innerHTML = `
          ...
          // else treat the child is a single link
          child.children[0].className = 'ud-menu-scroll mx-8 flex py-2 text-base font-medium text-dark group-hover:text-primary dark:text-white lg:mr-0 lg:inline-flex lg:px-0 lg:py-6 lg:text-white lg:group-hover:text-white lg:group-hover:opacity-70';
        }
      }
      i += 1;
    }
  ...
  navWrapper.className = 'absolute top-0 left-0 z-40 flex items-center w-full bg-transparent ud-header';
  navWrapper.innerHTML = `
  <div class="container">
    <div class="relative flex items-center justify-between -mx-4">
      <div class="max-w-full px-4 w-60">
        <a href="/" class="block w-full py-5 navbar-logo">
          <img src="/assets/logo/logo.svg" alt="logo" class="w-full dark:hidden" />
          <img src="/assets/logo/logo-white.svg" alt="logo" class="hidden w-full dark:block"/>
        </a>
      </div>
